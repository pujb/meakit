function [ handle ] = util_plot_8s_into_arraymap_bar( data, closed, haveBaseline)
...
for hwid = 1:64
    ...
    if ~(util_convert_hw2ch(hwid) == 11 || ...
        ...
        try
            h = bar(data.(['ch' num2str(util_convert_hw2ch(hwid))]).x, data.(['ch' num2str(util_convert_hw2ch(hwid))]).y, 'k');
            if ~haveBaseline
                set(get(h(1),'Baseline'),'LineStyle','none');
            end
            hold on;
            errorbar(data.(['ch' num2str(util_convert_hw2ch(hwid))]).x, data.(['ch' num2str(util_convert_hw2ch(hwid))]).y, data.(['ch' num2str(util_convert_hw2ch(hwid))]).e,'k','LineStyle','none');
            hold off;
        catch
