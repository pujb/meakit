function info = export(p, varargin)
...
% pre-exporting render
drawnow
subsasgn(p, 'print', true); % renderer will know to do anything print-specific
w = warning('off');
render(p)
drawnow

% export
print(p.fig, dev, ['-r' int2str(pars.dpi)], pars.file)
warning(w);
subsasgn(p, 'print', false);
drawnow

% re-render on screen, so it looks ok again on screen
render(p)
drawnow

% restore autorender setting
subsasgn(p, 'autorender', ar);

% info
if nargout
	info = [];
	info.sizemm = round(sz);
	info.sizepx = round(psz);
end
