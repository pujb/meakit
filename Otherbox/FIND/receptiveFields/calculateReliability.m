        ...
        if events
            ...
            for k=1:length(eventBeginnings{n})
                ...
                if find(in_window==2)
                    ...
                    strials=sort(theseSpikesTrials);
                    utrials=unique(strials);
                    for s=1:length(utrials)
                        temps(s)=length(find(strials==utrials(s)));
                    end
                    num=max(temps);
                    for uu=1:nTrials
                        spikesPerTrial(uu,:)=[theseSpikes(theseSpikesTrials==uu);zeros(num-length(theseSpikes(theseSpikesTrials==uu)),1)];
                    end
