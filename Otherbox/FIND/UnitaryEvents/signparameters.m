...
% Last Modified by GUIDE v2.0 13-Sep-2002 17:35:01
...
if isequal(varargin{1},'init') % LAUNCH GUI
    ...
    handles.text         = DataFile.SignFigure.Text;
    handles.filename     = DataFile.SignFigure.FileName;
    handles.allevents    = DataFile.EventList;
    handles.events       = DataFile.SignFigure.EventsToPlot;
    ...
    switch handles.cancel
     case 'off'
        ...
        information('Saved changes made in Significance Figure Parameters menu.');
        drawnow;
     case 'on'
        DataFile = handles.originaldata;
        
        information('No changes applied in Significance Figure Parameters menu.');
        drawnow;
...
function varargout = position_listbox_Callback(h, eventdata, handles, varargin)
 ...
 information('You can change all values in the edit fields.');
 drawnow;
...
function varargout = addposition_pushbutton_Callback(h, eventdata, handles, varargin)
  ...
  information(['Added new default line to "Vertical Lines". You must set the desired parameters ' ...
               'for the new line in the edit fields.']);
  drawnow;
...
function varargout = resetlist_pushbutton_Callback(h, eventdata, handles, varargin)
 ...
 information('Reset "List of Events to plot" to "List of all selected Events".');
 drawnow;
...
function varargout = patsel_popupmenu_Callback(h, eventdata, handles, varargin)
 ...
 switch pos
  case 1
     set(handles.pattern_edit,'enable','off');
     information('All possible patterns are plotted.');
     drawnow;
  case 2
...
function varargout = uemwa_pushbutton_Callback(h, eventdata, handles, varargin)
 ...
 information('Loaded "Vertical Lines" Settings from UEMWA Figure Parameters.');
 drawnow;
...
function varargout = text_pushbutton_Callback(h, eventdata, handles, varargin)
  ...
  if ~isempty(handles.datafilename) & ~isempty(handles.cutevent)
    [pathname, filename] = SplitFileName(handles.datafilename);
    text = [filename ' (pattern: ... , cut event: ...)'];
    set(handles.text_edit,'string',text);
    guidata(h,handles);
    information('Applied Default Text.');
    drawnow;
 else
    information(['Default Settings for Figure Text can only be applied if you ' ...
                 'have choosen a valid "Data File" and "Cut Event".']);
    drawnow;
 end
...
function varargout = filename_pushbutton_Callback(h, eventdata, handles, varargin)
 ...
 if ~isempty(handles.datafilename) & ~isempty(handles.dataoutpath)
    [pathname, filename] = SplitFileName(handles.datafilename);
    name = [handles.dataoutpath strrep(filename,'-','_') 'SignFigure'];
    set(handles.filename_edit,'string',name);
    information('Applied Default File Name.');
    drawnow;
 else
    information(['Default Settings for Figure File Name can only be applied if ' ...
                 'you have choosen a valid "Data File" and "Results File".']);
    drawnow;   
 end
