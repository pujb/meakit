...
% -------------------------------------------------------------------------
            ...
            % *************************************************************
            disp(' TO DO (SG, 8.8.2003) include option for randomizing windows')
            % In order to improve load balancing, it may be helpful to
            % randomize window ids, such that windows that take longer due
            % to higher rates are rather distributed across the processors.
            % How:
            % 1) allow for selection of randomizing or not
            % 2) allow for using a same, stored sequence of window ids
            % 3) give as parameter to CreateMakeFile.m the sequence,
            % or the order to permute (randperm) within CreateMakeFile.m
            % *************************************************************
...
% MSK ---------------------------------------------------------------------
            ...
            system(sprintf('%s/toolbox/compiler/deploy/%s/extractCTF %s/UEMainAnalyseWindow.ctf',...
                   matlabroot,getenv('ARCH'),getenv('PWD')));
            
            CreateMakeFile(uw.Results.ScratchDirName,...
...
% MSK ---------------------------------------------------------------------
          ...
          % * into a single temporary scratch directory                 *
          % *************************************************************
          
          host_list   = dir('/global.scratches/');
          tmp_scratch = sprintf('%s/tmp_scratch/',getenv('HOME'));
          try
              mkdir(tmp_scratch)
          catch
          end
          for x = 3:length(host_list)
              try
                  movefile(sprintf('/global.scratches/%s/%s/UEResultsOfJob*',host_list(x).name,getenv('USER')),tmp_scratch)
              catch
                  disp(sprintf('no result file(s) in /global.scratches/%s/%s/',host_list(x).name,getenv('USER')))
              end
          end
          uw.Results.ScratchDirName = tmp_scratch;
% -------------------------------------------------------------------------
...
% MSK ---------------------------------------------------------------------
          ...
          try
              rmdir(uw.Results.ScratchDirName,'s')
          catch
              disp(lasterr)
          end
% -------------------------------------------------------------------------
