...
% catching. It was designed to retrieve data (not info or anything else)
% from a single entityID (and in case of segment data a single segment/sweep).
        ...
        case {'pike'} % could be meabench .spike file
            if strcmp(lower(fileName(end-5:end)),'.spike')
                loadNoNsFile('fileName', fileName,'segmentData', segmentData,...
                    'eventData',eventData,'neuralData',neuralData);
                DLLName = 'none neuroshare import';
            end
        case {'.gdf','.mat'}
            loadNoNsFile('fileName', fileName,'analogData',analogData,'segmentData', segmentData,...
                'eventData',eventData,'neuralData',neuralData)
            DLLName = 'none neuroshare import';
        otherwise
