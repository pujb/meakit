...
optionalArgs={{'selected_Process', @(val) ismember(val,{'rasterPlot','PSTH',...
    'JPSTH','filter','ISI distribution','Correlation','Coherence','FFT','PCA','ICA'})}};
...
try
    ...
    radioButtonFilter = uicontrol('Style','Radio',...
        ...
        radioButtonISI = uicontrol('Style','Radio',...
            'String','ISI distribution',...
            'Units','characters',...
            'pos',[1 18.5 20 BUTTON_HEIGHT],...
            'parent',radioGroup,...
            'HandleVisibility','on',...
            'Enable','off',...
            'Tag','processSelectionGUI_radioButtonISI');
    ...
    switch selected_Process
        ...
        case 'filter'
         ...
         case 'ISI distribution'
            set(radioGroup,'SelectedObject',[radioButtonISI]);
           set(radioGroup,'UserData','ISI distribution');
  %          uicontrol('Parent',rightPanel,...
%                 'Units','characters',...
%                 'Position',[10 8 50 LABEL_HEIGHT],...
%                 'Style','text',...
...
function selectfilterType_callback(source,eventdata)

% Get the Value from the Radio Button-GroupfilterType
temp=get(get(source,'SelectedObject'),'String');
tmp=findobj('Tag','filterTypeSelectionGUI_radioGroupfilterType');
set(tmp,'UserData',temp);
selected_filterType=get(tmp(1),'UserData');

%%%%%%%%%%%%%%%%%%%%%%%%
switch selected_filterType
    case 'Savgol'
        % Frame Size
        tmp=findobj('Tag','Input1_txt');
        set(tmp,'Enable','on','String','Frame Size');
        tmp=findobj('Tag','Input1_edit');
        set(tmp,'Enable','on','String','[19]');        
        
        % Polynomial Order
        tmp=findobj('Tag','Input2_txt');
        set(tmp,'Enable','on','String','Polynomial Order');
        tmp=findobj('Tag','Input2_edit');
        set(tmp,'Enable','on','String','[7]');   
        
    case {'Lowpass' , 'Highpass'}
        % Frame Size
        tmp=findobj('Tag','Input1_txt');
        set(tmp,'Enable','on','String','Cut off Frequency');
        tmp=findobj('Tag','Input1_edit');
        set(tmp,'Enable','on','String','[1000]');
        
        tmp=findobj('Tag','Input2_txt');
        set(tmp,'Enable','off','String','');
        tmp=findobj('Tag','Input2_edit');
        set(tmp,'Enable','off','String','');
    ...
    case {'Bandpass','Bandstop'}
         % Frame Size
        tmp=findobj('Tag','Input1_txt');
        set(tmp,'Enable','on','String','Start Frequency');
        tmp=findobj('Tag','Input1_edit');
        set(tmp,'Enable','on','String','[100]');
        
        tmp=findobj('Tag','Input2_txt');
        set(tmp,'Enable','on','String','Stop Frequency');
        tmp=findobj('Tag','Input2_edit');
        set(tmp,'Enable','on','String','[1000]');
end
%%%%%%%%%%%%%%%%%%%%%%%%%

postMessage([selected_filterType, ' selected ']);
...
try
    ...
    temp1=findobj('Tag','dataID_edit');
    userString = get(temp1(1),'string');
    filterID=str2num(char(userString));

    
    temp2=findobj('Tag','Input1_edit');
    userString = get(temp2(1),'string');
    Input1 = str2num(char(userString));
    
    temp2=findobj('Tag','Input2_edit');
    userString = get(temp2(1),'string');
    Input2 = str2num(char(userString));
    
    tmpfilterType=findobj('Tag','filterTypeSelectionGUI_radioGroupfilterType');
    selected_filterType=get(tmpfilterType(1),'UserData');
    
    %tmpNorm=findobj('Tag','normSelectionGUI_radioGroup2');
    %selected_Norm=get(tmpNorm(1),'UserData');
    
    % entity check
    if ~any(ismember(filterID,nsFile.Analog.DataentityIDs))&&...
            ~any(ismember(filterID,nsFile.Segment.DataentityIDs))
        postMessage('Please load data first!');
    else
        % passing on the varialbles
        filterPlot('selected_filterType',selected_filterType,'selectedidx',...
            filterID, 'Input2',Input2,'Input1',Input1);
