...
try
    ...
    if setInfos
        ...
        % rewrite Analog.Info.ItemCount in case of sequential loading
        % do it here - before zero padding - then ItemCount contains the
        % effective number of Items (without zero padding)
        for vv=1:size(newdata.Analog.Info,1)
            newdata.Analog.Info(vv).ItemCount=size(newdata.Analog.Data,1);
        end
    end
