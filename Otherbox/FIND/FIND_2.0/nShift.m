function [shifts] = nShift(data,samplingRate,maxlagms,normflag,parabolicflag,parabolicsigma)
% function [shifts] = nShift(data,samplingRate,maxlagms,normflag,parabolicflag,parabolicsigma);
% Parameter List:
...
if ~ismember(normflag,{'none','coeff','unbiased','biased'})
    error('FIND:invalid input argument for normflag')
end

if ~ismember(parabolicflag,{'lin','log'})
    error('FIND:invalid input argument for parabolicflag')
end
