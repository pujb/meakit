...
for ii=1:no_props
    geometry{1}(ii+1)=length(propnames{ii});
end
geometry{1}(1)=4;
...
for ii = 1:length(pageentries)
    ...
    for jj=1:no_props
        if jj==1
            ...
            %             % replace EntityLabel with EntityID if the string is empty
            %             if (isempty(nsFile.EntityInfo(pageentries(ii)).EntityLabel))
            %                 listui{length(listui)+1}={'style','text','string',strcat('EntityID: ',num2str(nsFile.EntityInfo(pageentries(ii)).EntityID))};
            %             else
            %                 listui{length(listui)+1}={'style','text','string',nsFile.EntityInfo(pageentries(ii)).EntityLabel};
            %             end
        end
