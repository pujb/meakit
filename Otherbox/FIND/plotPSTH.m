...
%              'KernelSize',[0.01],'selectedNorm','normalised',...
%               'selectedDirect',(-1));
%
    ...
    if overview~=0
        ...
        for kk=eventID
            for ii=neuronID
                ...
                scrsz = get(0,'ScreenSize');
                if nUnits>2
                    fh=figure('Name',['PSTH - neuronID',num2str(ii)],'NumberTitle','off','Units','normalized','Position',[0.25 0.25 .5 .5]);
                else
                    fh=figure('Name',['PSTH - neuronID',num2str(ii)],'NumberTitle','off');
                end
                ...
                if ~strcmp(selectedKernel,'BIN')
                    ...
                    for uu=1:nUnits
                        ...
                        stairs(-preTrigger:(preTrigger+postTrigger)/(length(spVec)-1):postTrigger,spVec);
