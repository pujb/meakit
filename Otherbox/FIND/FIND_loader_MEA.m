function output=FIND_loader(varargin)
...
% memory.
%
% LOADING DATA DIRECTLY TO A VARIABLE
% this is used for the preview data function and uses much less error
% catching. It was designed to retrieve data from a single entityID (and
% in case of segment data a single segment/sweep). It can also load
% fileinfo, entityinfo and segmentinfo.
% it will probably work only for this purpose. to use it, assign an output
% variable to the function and set the pvp "% 'tovar',1 ".
% (as of 30/7/09)
%
% (0) Markus Nenniger 06/07, a.kilias 08/08 major revision
% loading to output variable added 07/2009  Markus Nenniger
...
try
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%% FileInfo %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if fileInfo ~= 0;
        [ns_RESULT, tmpinfo] = ns_GetFileInfo(hfile);
        if tovar==1
            output=tmpinfo;
            clear mexprog;
            return;
        end
        nsFile.FileInfo=tmpinfo;
        clear tmpinfo;
    end
    ...
    if entityInfo ~= 0;
        ...
        if tovar==1
            if strcmp(entityInfo,'all')
                ...
                entityInfo=1:tempfileinfo.EntityCount;
            end
            [ns_RESULT,output]=ns_GetEntityInfo(hfile, entityInfo);
            clear mexprog;
            return
        end
        if strcmp(entityInfo,'all')
    ...
    if segmentInfo ~= 0;
        ...
        elseif strcmp(segmentInfo,'all') && ~isempty(segmententityIDs)
            [ns_RESULT, tmpseginfo] = ns_GetSegmentInfo(hfile, segmententityIDs);
            % add Field "EntityID"
            nsFile.Segment.Info=tmpseginfo;
        ...
        elseif all(ismember(segmentInfo,segmententityIDs))
            [ns_RESULT, tmpseginfo] = ns_GetSegmentInfo(hfile, segmentInfo);
            if tovar==1
                output=tmpseginfo;
                clear mexprog;
                return;
    ...
    if analogData ~= 0;
        ...
        if length(differentlengths)>1 % entities are not of equal lengths
            for kk=length(differentlengths):-1:1
                ...
                if any(LastIdx>[tempentityinfo(IDstoload(find(uniqueidx==kk,1))).ItemCount])
                    LastIdx=tempentityinfo(IDstoload(find(uniqueidx==kk,1))).ItemCount;
                end
                [ns_RESULT, ContCount, newdata.Analog.Data] = ns_GetAnalogData(hfile,samelengthIDs,FirstIdx,LastIdx-FirstIdx+1);
            ...
            if tovar==1
                output=newdata.Analog.Data;
                %normal precautions...
                clear mexprog
                
                %why does this cause problems? shouldn't... MN 090702
