...
set(myguiprops.mygui,'DeleteFcn','savewindowposition');
...
% listui{1,size(listui,2)+1}={'style','text','string',''};
listui{1,size(listui,2)+1}={'style','text','string','Interval [sampl. pts] :'};
listui{1,size(listui,2)+1}={'style', 'edit' , 'String','[]','Tag','setSequeditbox'};
...
%get windowsize from when it was last opened
if isempty(FIND_GUIdata.GUIpos.browseEntitiesGUI)
    FIND_GUIdata.GUIpos.browseEntitiesGUI=get(myguiprops.mygui,'Position');
else
    %recall position
    set(myguiprops.mygui,'Position',FIND_GUIdata.GUIpos.browseEntitiesGUI);
end


%check selection checkboxes for selected entityIDs
if ~isempty(FIND_GUIdata.IDselected==1)
    visibleID = intersect(find(FIND_GUIdata.IDselected==1),myguiprops.pageentries);
    if (~isempty(visibleID))
        for selectedID=visibleID
...
try
    ...
    % via GUI only analog data
    loadSequence=eval(get(findobj(gcf,'Tag','setSequeditbox'),'String'));
    ...
    for ii=1:4
        ...
        elseif ii==2 %set pvp string for analog data
            ...
            %store loading parameters for FIND_loader (see FIND_loader for details)
            if ~isempty(loadSequence)
                pvpstring{end+1}=[min(loadSequence) max(loadSequence) tempidx];
            else
